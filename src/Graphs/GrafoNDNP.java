...
public class GrafoNDNP {
	...
	public GrafoNDNP(int capacidad) {
		...
		ma = new byte[capacidad][capacidad];
	...
	public void eliminarNodosTerminales() {
		for (int i = 0; i < n; i++) {
			String x = id[i];
			if (grado(x) == 1) {
				eliminarVertice(x);
	...
	public void pruebaIncostitencia() {
		ma[0][0] = 1;
	}
	...
	public boolean insertarVertice(String nombre) throws grafoInconsistenteException {
	...
	public boolean insertarArista(String origen, String destino) {
		if (existeVertice(origen) && existeVertice(destino)) {
			ma[indiceVertice(origen)][indiceVertice(destino)] = 1;
			ma[indiceVertice(destino)][indiceVertice(origen)] = 1;
			return true;
		} else
			return false;
	}
	...
	public boolean existeArista(String origen, String destino) {
		if (existeVertice(origen) && existeVertice(destino)) {
			return ma[indiceVertice(origen)][indiceVertice(destino)] == 1;
		} else
			return false;
	}
	...
	public boolean eliminarArista(String origen, String destino) {
		if (existeVertice(origen) && existeVertice(destino)) {
			if (ma[indiceVertice(origen)][indiceVertice(destino)] == 1) {
				ma[indiceVertice(origen)][indiceVertice(destino)] = 0;
				ma[indiceVertice(destino)][indiceVertice(origen)] = 0;
				return true;
			} else
				return false;
		} else
		...
		if (existeVertice(origen)) {
			...
			for (int i = 0; i < n - 1; i++) {
				...
				while (!l.isEmpty()) {
					...
					if (!marcados[ind1] && distancias[ind] + 1 < distancias[ind1]) {
						distancias[ind1] = distancias[ind] + 1;
						rutas[ind1] = ind;
					}
				}
				int indice = -1, distancia = Integer.MAX_VALUE;
				for (int j = 0; j < n; j++) {
	...
	public GrafoNDNP getComplemento(GrafoNDNP a) {
		GrafoNDNP x = a;
		for (int i = 0; i < x.capacidad; i++) {
			for (int j = 0; j < x.capacidad; j++) {
				if (ma[i][j] == 0) {
					ma[i][j] = -1;
				}
			}
